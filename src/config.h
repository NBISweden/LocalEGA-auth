...
struct options_s {
  ...
  /* NSS & PAM queries */
  const char* get_ent;        /* SELECT elixir_id,'x',<uid>,<gid>,'EGA User','/<fuse_mnt>/'|| elixir_id,'/bin/bash' FROM users WHERE elixir_id = $1 */
  ...
  const char* get_account;    /* SELECT password_hash FROM users WHERE elixir_id = $1 */
